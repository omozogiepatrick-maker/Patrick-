...
function seedData() {
  ...
  const decisions = [
    { id: uid("d"), machineId: machines[0].id, risk: "Critical", downtimeCost: 15000, action: "Replace cooling pump today", reason: "Temperature rising rapidly with no sign of plateau; cooling pump output has degraded 30% over the last 2 weeks.", confidence: 92, status: "Pending", decisionReason: "", decidedBy: "", decidedAt: null, workOrderId: null },
    { id: uid("d"), machineId: machines[2].id, risk: "High", downtimeCost: 8200, action: "Inspect and reset compressor trip circuit", reason: "Two unexplained breaker trips this shift; recurring trip faults historically precede full compressor failure.", confidence: 81, status: "Pending", decisionReason: "", decidedBy: "", decidedAt: null, workOrderId: null },
    { id: uid("d"), machineId: machines[1].id, risk: "Medium", downtimeCost: 1500, action: "Schedule bearing inspection within 3 days", reason: "Vibration trending upward but still within tolerable range.", confidence: 68, status: "Pending", decisionReason: "", decidedBy: "", decidedAt: null, workOrderId: null },
  ];
  const workOrders = [
  ...
  const team = [{ id: uid("u"), name: "J. Alvarez", role: "Manager" }, { id: uid("u"), name: "R. Novak", role: "Technician" }, { id: uid("u"), name: "M. Chen", role: "Planner" }];
  const activity = [
...
function TodaysPriorityStrip({ decisions, machines, setTab }) {
  const top = decisions.filter((d) => d.status === "Pending").slice().sort((a, b) => RISK_RANK[a.risk] - RISK_RANK[b.risk]).slice(0, 3);
  if (top.length === 0) return null;
  return (
    <Card style={{ padding: 16, marginBottom: 20, background: "linear-gradient(180deg, #1B2127 0%, #191F25 100%)" }}>
      <div style={{ fontSize: 11, color: "#8A96A3", fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 10 }}>Today's Priorities</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {top.map((d) => {
          const m = machines.find((m) => m.id === d.machineId);
          const emoji = d.risk === "Critical" || d.risk === "High" ? "🔴" : d.risk === "Medium" ? "🟠" : "🟢";
          return (
            <button key={d.id} onClick={() => setTab("decisions")} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: "4px 0", color: "#EAEEF1" }}>
              <span style={{ fontSize: 15 }}>{emoji}</span>
              <span style={{ fontSize: 13.5, fontWeight: 600, flex: 1 }}>{d.action}</span>
              <span style={{ fontSize: 11, color: "#5B6672", fontFamily: F_MONO }}>{m?.name}</span>
              <ChevronRight size={14} color="#5B6672" />
            </button>
          );
        })}
      </div>
...
function Dashboard({ data, session, setTab }) {
  ...
  const aiRecommended = data.decisions.filter((d) => d.status === "Pending").length;
  ...
  return (
    <div>
      ...
      <TodaysPriorityStrip decisions={data.decisions} machines={data.machines} setTab={setTab} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 22 }}>
        {stats.map((s) => (
          <Card key={s.label} style={{ padding: "16px 18px" }}>
            <s.icon size={16} color={s.color} style={{ marginBottom: 10 }} />
            <div style={{ fontFamily: F_MONO, fontSize: 24, fontWeight: 600 }}>{s.value}{s.total !== undefined ? <span style={{ fontSize: 13, color: "#5B6672" }}> /{s.total}</span> : null}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 16, alignItems: "start" }}>
        <Card style={{ padding: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: F_DISPLAY, fontWeight: 600, fontSize: 15 }}><Sparkles size={16} color="#F5A623" /> AI Recommended Actions</div>
            <button onClick={() => setTab("decisions")} style={{ background: "none", border: "none", color: "#F5A623", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>Open Decision Center <ChevronRight size={13} /></button>
          </div>
          {aiRecommended === 0 ? <EmptyState icon={Sparkles} text="No pending recommendations. Run the Decision Center to analyze current conditions." /> : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {data.decisions.filter((d) => d.status === "Pending").sort((a, b) => RISK_RANK[a.risk] - RISK_RANK[b.risk]).slice(0, 4).map((d) => {
                const m = data.machines.find((m) => m.id === d.machineId);
                return (
                  <div key={d.id} style={{ display: "flex", gap: 12, padding: "10px 12px", background: "#12161A", borderRadius: 8, borderLeft: `3px solid ${RISK_COLOR[d.risk]}` }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: "#8A96A3" }}>{m?.name} · Est. impact {fmtMoney(d.downtimeCost)}</div>
                    </div>
                  </div>
                );
              })}
...
function buildAiPrompt(data) {
  ...
  return `You are the Decision Center inside MEO, a factory maintenance execution system. Your ONLY job: tell the maintenance manager what needs attention right now, why it matters, what happens if they wait, and what to do next — using ONLY the real registered machines and logged history given below. Never invent a machine, part, or event that is not in this data.

If a machine has little or no logged history (few or no alerts, no completed repairs), do not guess specifics — instead produce a recommendation with a clearly lower confidence score and say plainly in the reason that there isn't much history yet, rather than inventing a detailed diagnosis. Weight rising trends (alertsLast30Days, days past due on maintenance, repeated alert types) more heavily than a single event.
...
[{"machineId": "<the machine's machineId field, exactly as given>", "risk": "<Low, Medium, High, or Critical>", "downtimeCost": <integer USD estimate of cost if this is ignored, based on criticality and status — reasonable order-of-magnitude, not precise>, "action": "<short imperative action, e.g. 'Replace cooling pump today'>", "reason": "<1-2 sentences citing the specific real evidence — trend, sensor note, days overdue, etc>", "confidence": <integer 0-100, lower when history is sparse>}]
...
function DecisionCenter({ data, setData, session, logActivity }) {
  ...
  async function generate() {
    ...
    try {
      ...
      const newDecisions = recs.map((r) => {
        ...
        return { id: uid("d"), machineId: m ? m.id : null, risk: r.risk, downtimeCost: r.downtimeCost, action: r.action, reason: r.reason, confidence: r.confidence, status: "Pending", decisionReason: "", decidedBy: "", decidedAt: null, workOrderId: null };
  ...
  function confirmDecide(reasonText) {
    ...
    setData((d) => {
      ...
      if (action === "Approved") {
        ...
        const wo = {
          id: uid("wo"), woNumber, machineId: decision.machineId, assignedTech: "Unassigned",
          priority: decision.risk, dueDate: new Date().toISOString(), requiredParts: "", estimatedHours: null,
          status: "Pending", decisionId: decision.id, technicianNotes: "", startedAt: null, completedAt: null, actualCost: null,
        };
  ...
  return (
    <div>
      <PageHeader title="Decision Center" subtitle="What needs attention right now, why it matters, and what to do next." action={
            ...
            return (
              <Card key={d.id} style={{ padding: 22, borderLeft: `4px solid ${color}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14, gap: 12, flexWrap: "wrap" }}>
                  ...
                  <Badge color={color}>{d.risk} risk</Badge>
                </div>
      ...
      {decideModal && <DecideModal decision={decideModal.decision} action={decideModal.action} machine={data.machines.find((m) => m.id === decideModal.decision.machineId)} onClose={() => setDecideModal(null)} onConfirm={confirmDecide} />}
    </div>
  );
}

function DecideModal({ decision, action, machine, onClose, onConfirm }) {
  ...
  return (
    <Modal title={labels[action]} onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {action === "Approved" && <div style={{ fontSize: 12.5, color: "#8A96A3" }}>This creates a new work order automatically, ready to assign to a technician.</div>}
        ...
        <Button style={{ justifyContent: "center", background: colors[action], border: `1px solid ${colors[action]}`, color: "#0C0F12" }} disabled={!reason.trim()} onClick={() => onConfirm(reason.trim())}>Confirm {action.toLowerCase()}</Button>
      </div>
    </Modal>
  );
}
...
function SettingsPage({ data, setData, session, logActivity }) {
  ...
  function addMember() { if (!newMember.name.trim()) return; setData((d) => ({ ...d, team: [...d.team, { id: uid("u"), ...newMember }] })); setNewMember({ name: "", role: "Technician" }); logActivity(`Added team member ${newMember.name} (${newMember.role})`); }
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Card style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12, maxHeight: 160, overflowY: "auto" }}>
            {data.team.map((u) => (
              <div key={u.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12.5, padding: "6px 0", borderBottom: "1px solid #2B333B" }}>
                <span>{u.name} <span style={{ color: "#8A96A3" }}>· {u.role}</span></span>
          ...
          {canEdit && (
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <Button variant="ghost" onClick={addMember}><Plus size={14} /></Button>
            </div>
          )}
        </Card>
        ...
        <Card style={{ padding: 20 }}>
          ...
          <div style={{ fontSize: 12, color: "#8A96A3" }}>Sensor/IoT integrations are future-ready — Alerts already has a field for sensor data; live feeds would connect here.</div>
        </Card>
      </div>
    </div>
  );
